...
export const StudentView: React.FC<StudentViewProps> = ({ canteen: initialCanteen }) => {
  ...
  useEffect(() => {
    ...
    const updateStatus = async () => {
      try {
        ...
        const pos = await BackendService.getQueuePosition(activeCanteen.id, token.id, QueueType.CAMPUS);
        if (updatedToken) {
  ...
  const placeOrder = async (canteenId: string) => {
    ...
    try {
      ...
      const newToken = await BackendService.createToken(canteenId, ticketHash, selectedFood, QueueType.CAMPUS);
      setToken(newToken);
      const pos = await BackendService.getQueuePosition(canteenId, newToken.id, QueueType.CAMPUS);
      setQueuePosition(pos);
