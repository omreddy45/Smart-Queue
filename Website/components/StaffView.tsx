...
import { CheckCircle, Clock, Check, Utensils, AlertCircle, MapPin, Sparkles, LogOut } from 'lucide-react';
...
export const StaffView: React.FC<StaffViewProps> = ({ canteen, onLogout }) => {
  ...
  useEffect(() => {
    ...
    const handleQueueUpdate = () => {
      console.log('[StaffView] Queue update event received');
      fetchQueue();
    };

    window.addEventListener('smartqueue-update', handleQueueUpdate);
    
    // Also listen to storage changes (for cross-tab updates)
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'smartqueue_tokens') {
        console.log('[StaffView] Storage changed, fetching queue');
        fetchQueue();
      }
    };
    ...
    return () => {
      window.removeEventListener('smartqueue-update', handleQueueUpdate);
